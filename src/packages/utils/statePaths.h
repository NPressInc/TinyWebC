 ...
 * All state data (storage, database, keys) is stored under state/
 * Each container/node has its own isolated state directory
 */
...
typedef struct {
    ...
    char base_dir[MAX_STATE_PATH_LEN];           // state/
    char blockchain_dir[MAX_STATE_PATH_LEN];     // state/storage
    char keys_dir[MAX_STATE_PATH_LEN];           // state/keys
    char blockchain_file[MAX_STATE_PATH_LEN];    // state/storage/blockchain.dat
    char blockchain_json[MAX_STATE_PATH_LEN];    // state/storage/blockchain.json
    char database_file[MAX_STATE_PATH_LEN];      // state/storage/tinyweb.db
    char private_key_file[MAX_STATE_PATH_LEN];   // state/keys/node_private.key
} NodeStatePaths;
 ...
 * @param debug_mode Deprecated, kept for compatibility (no longer affects paths)
...
/**
 * Get the storage directory for a node
 *
